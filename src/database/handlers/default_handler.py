...
class DefaultTableHandler:
    ...
    def _extract_table_multipliers(self, sheet, table_loc: TableLocation, table_id: int, conn):
        """Extract multipliers for each height row and width column"""
        cursor = conn.cursor()
        ...
        try:
            ...
            # Extract multipliers for each height row (width exceeded multipliers - regular and WD)
            for row in range(table_loc.width_row, end_row, step):
                height = self.is_inch_value(sheet.cell(row, table_loc.start_col).value)
                if height is None:
                    continue
                
                # Check for regular width exceeded multiplier in the column to the right of the table
                width_mult_cell = sheet.cell(row, table_loc.end_col + 1).value
                ...
                width_mult_wd_cell = sheet.cell(row + 1, table_loc.end_col + 1).value
            ...
            # Extract multipliers for each width column (height exceeded multipliers - regular and WD)
            for col in range(table_loc.height_col, table_loc.end_col + 1):
                width = self.is_inch_value(sheet.cell(table_loc.start_row, col).value)
                if width is None:
                    continue
                
                # Check for regular height exceeded multiplier in the row below the table
                height_mult_cell = sheet.cell(table_loc.end_row + 1, col).value
                ...
                height_mult_wd_cell = sheet.cell(table_loc.end_row + 2, col).value
